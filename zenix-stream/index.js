...
/* Zenix — Synthetiq Player video module (contract v3).
 * Source: https://zenix.best (FR films / series; NO anime catalogue:
 * https://zenix.best/animes and /anime/* return 404, homepage has no anime).
 * Runtime: isolated JS with global `fetchv2`. No Node.js, no DOM.
 * Evidence (2026-09-20, unauthenticated GET):
 *  - Search:  GET /search/{encodeURIComponent(query)} -> HTML `zenix-card`
 *             blocks (data-watch/data-title/data-year/data-genres) linking to
 *             /movie/{slug}, /tv-show/{slug}. `?s=` returns the homepage,
 *             /search?q= and /api/search are 404; no JSON API, no auth/POST.
 *  - Details: GET /movie|/tv-show/{slug} -> <h1>, meta description,
 *             og:image, /genre/ links, /episode/ links (series).
 *  - Episode: GET /episode/{slug}/{season}-{episode} -> watch-component
 *             snapshot videos=[{label,type,link,provider}] e.g. BlinkFlux HD
 *             (mp4 via /embed/46980?v=4), Lecteur Gratuit 4K (first-party
 *             iframe embed), Peachify (third-party embed). Embed pages return
 *             200 HTML with an empty <video id="player"> (source injected by
 *             encrypted runtime JS after gates); page gated by Turnstile.
 *             /videos/tutorial-*.mp4 are tutorial clips, NOT title media.
 *             This module does NOT bypass CAPTCHA / gates / login / DRM:
 *             extractStreamUrl only returns a stream after fetching the
 *             candidate media URL and verifying a non-HTML video payload.
 */

var BASE_URL = 'https://zenix.best';
...
/* Parse detail cards (/movie|/tv-show) from any listing HTML.
 * Fast path: zenix-card blocks expose data-watch (detail URL), data-title,
 * data-year and data-genres attributes. Fallback: generic link scan. */
function parseCards(html) {
  var out = [];
  var seen = {};
  function escHost(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
  function pushCard(url, title, image, year, genre) {
    if (!url || seen[url]) return;
    seen[url] = true;
    var t = (title && String(title).trim()) || null;
    if (!t || /^(film|serie|s[eé]rie|anim[eé]|saison|episode)$/i.test(t)) {
    ...
    out.push({
      ...
      type: kind, kind: kind, year: year || null, genre: genre || null
    });
  }
  var zcard = /<div[^>]*class="[^"]*zenix-card[^"]*"[^>]*>/g;
  var zm;
  while ((zm = zcard.exec(html))) {
    var tag = zm[0];
    var w = tag.match(/data-watch="(https:[^"]+)"/);
    if (!w) continue;
    var url = w[1];
    var dt = tag.match(/data-title="([^"]{1,200})"/);
    var dy = tag.match(/data-year="(\d{4})"/);
    var dg = tag.match(/data-genres="([^"]{0,200})"/);
    var after = html.slice(zm.index, Math.min(html.length, zm.index + 2500));
    var img = after.match(/data-src="(https:[^"]+?)"/);
    var image = (img && img[1].indexOf('data:image') !== 0) ? img[1].split('?')[0] : null;
    if (!image) image = pickImage(after);
    var genres = dg ? decodeEntities(dg[1]).split('|').map(function (g) { return g.trim(); }).filter(Boolean).join(', ') : null;
    pushCard(url, dt && decodeEntities(dt[1]), image, dy ? dy[1] : null, genres);
  }
  var hostRe = new RegExp('(' + escHost(BASE_URL) + '\\/(?:movie|tv-show)\\/[a-z0-9\\-]+)', 'g');
  var m;
  while ((m = hostRe.exec(html))) {
    var u2 = m[1];
    if (seen[u2]) continue;
    var ctx = html.slice(m.index, Math.min(html.length, m.index + 3500));
    var title = null;
    ...
    pushCard(u2, title, pickImage(ctx), (ctx.match(/\b((?:19|20)\d{2})\b/) || [])[1] || null, null);
  }
  return out;
}
...
/* Providers declared by the watch-component snapshot, e.g.
 * [{label:"BlinkFlux HD", type:"mp4", link:"https://zenix.best/embed/..",
 *   provider:"fastflux"}]. Legacy {label,type,index,source} shape kept as
 * fallback. */
function parseDeclaredVersions(html) {
  var versions = [];
  var re = /&quot;label&quot;:&quot;([^&]+?)&quot;,&quot;type&quot;:&quot;([^&]+?)&quot;,&quot;link&quot;:&quot;([^&]+?)&quot;,&quot;provider&quot;:&quot;([^&]+?)&quot;/g;
  var m;
  while ((m = re.exec(html))) {
    versions.push({
      label: decodeEntities(m[1]).replace(/\\u00[a-z0-9]+/gi, '').replace(/\\/g, ''),
      type: m[2],
      link: decodeEntities(m[3]).replace(/\\\//g, '/').replace(/&amp;/g, '&'),
      provider: m[4]
    });
  }
  var reOld = /&quot;label&quot;:&quot;([^&]+?)&quot;,&quot;type&quot;:&quot;([^&]+?)&quot;,&quot;index&quot;:(\d+),&quot;source&quot;:&quot;([^&]+?)&quot;/g;
  while ((m = reOld.exec(html))) {
    versions.push({
      label: decodeEntities(m[1]).replace(/\\/g, ''),
      type: m[2], index: parseInt(m[3], 10), source: m[4], link: null, provider: null
    });
  }
  return versions;
}
...
function findMediaUrls(html) {
  ...
  function push(u, via) {
    ...
    /* Tutorial clips are site chrome, not title media — never playable. */
    if (/\/videos\/tutorial-[0-9]+\.mp4/i.test(url)) return;
    if (seen[url]) return;
    seen[url] = true;
...
globalThis.extractDetails = async function (id) {
  try {
    ...
    /* Bare slug: probe the detail routes in order (no anime on zenix). */
    var probes = [BASE_URL + '/tv-show/' + url, BASE_URL + '/movie/' + url];
...
globalThis.extractStreamUrl = async function (episodeHref, lang) {
  ...
  var label = wanted.length ? wanted[0].label : 'Lecteur';
  var provider = (wanted[0] && wanted[0].provider) || null;
  var candidates = findMediaUrls(html);

  /* Declared provider links (first/third-party embeds) are resolved
   * dynamically: fetched live and scanned for mp4/m3u8, then verified. */
  var declared = [];
  for (var d = 0; d < wanted.length; d++) {
    if (wanted[d].link && /^https?:\/\//i.test(wanted[d].link)) {
      declared.push({ url: wanted[d].link, via: 'provider:' + (wanted[d].provider || wanted[d].type || 'embed') });
    }
  }
  candidates = declared.concat(candidates);

  /* One level of dynamic resolution: provider links and non-YouTube
   * iframe embeds are fetched and scanned for mp4/m3u8 (temporary links
   * resolved live). HTML/embed pages that expose no media are rejected. */
  var embeds = candidates.filter(function (c) { return c.via === 'iframe' || c.via.indexOf('provider:') === 0; }).slice(0, 5);
  ...
  for (var k = 0; k < direct.length; k++) {
    ...
    try {
      ...
      var stream = {
        ...
        source: provider || (wanted[0] && wanted[0].source) || 'local',
        via: direct[k].via
      };
  ...
  if (!direct.length) {
    throw new Error(
      ...
      ' (declared providers: ' + JSON.stringify(versions.map(function (v) { return v.label + '@' + (v.provider || v.type); })) +
      '). Playback requires the site gate (Turnstile/client-side player) and was not bypassed.' +
      (errors.length ? ' Checks: ' + errors.join(' | ') : '')
    );
  }
  throw new Error(
    'STREAM_GATED: all ' + direct.length + ' candidate(s) on ' + url +
    ' failed verification (declared providers: ' + JSON.stringify(versions.map(function (v) { return v.label + '@' + (v.provider || v.type); })) +
    '). Playback requires the site gate (Turnstile/client-side player) and was not bypassed.' +
    (errors.length ? ' Checks: ' + errors.join(' | ') : '')
  );
};
